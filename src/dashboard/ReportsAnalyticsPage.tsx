  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      if (isOnline()) {
        ...
        chemicals = chemicalsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        // Merge in local records not yet synced
        try {
          const localPayments = await getAllLocal("payments") as any[] || [];
          ...
          const localServices = await getAllLocal("services") as any[] || [];
          ...
          const localChemicals = await getAllLocal("chemicals") as any[] || [];
          for (const local of localChemicals) {
            const match = chemicals.find(f =>
              f.name === local.name
            );
            if (!match) chemicals.push(local);
          }
        } catch {}
      } else {
        // Offline: load from IndexedDB only
        payments = await getAllLocal("payments") as any[] || [];
        services = await getAllLocal("services") as any[] || [];
        chemicals = await getAllLocal("chemicals") as any[] || [];
      }
