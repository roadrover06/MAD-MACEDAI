...
const CommissionsPage: React.FC<{
  ...
  // Fetch employees and payments (offline + online)
  const fetchAll = async () => {
    ...
    try {
      ...
      const firestoreEmployees = empSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Employee[];
      const firestorePayments = paySnap.docs.map(doc => ({ id: doc.id, ...doc.data() })) as PaymentRecord[];
      // Local offline
      let localEmployees: Employee[] = [];
      let localPayments: PaymentRecord[] = [];
      try {
        localEmployees = await getAllLocal("employees") as Employee[] || [];
        localPayments = await getAllLocal("payments") as PaymentRecord[] || [];
      } catch {}
      // Merge employees
      const mergedEmployees: Employee[] = [...firestoreEmployees];
      for (const local of localEmployees) {
        const match = firestoreEmployees.find(f =>
          f.firstName === local.firstName &&
          f.lastName === local.lastName
        );
        if (!match) mergedEmployees.push(local);
      }
      ...
      for (const local of localPayments) {
        const match = firestorePayments.find(f =>
          ...
          f.price === local.price &&
          f.serviceName === local.serviceName
        );
  ...
  // Pagination for filtered commissions
  const paginated = filtered.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);

  // Reset to first page when filtered changes
  useEffect(() => {
    setPage(0);
  }, [filtered.length, search, dateFrom, dateTo]);
