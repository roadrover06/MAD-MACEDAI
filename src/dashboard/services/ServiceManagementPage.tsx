...
const ServiceManagementPage: React.FC<ServiceManagementPageProps> = ({
  ...
  const handleAddService = async () => {
    const errors = {
      ...
      // Allow description to be empty, so don't require it
      description: false,
    };
    ...
    try {
      await addService({
        ...newService,
        // If description is empty, store as empty string (or you can omit if you want)
      ...
      setAddFormErrors({ name: false, description: false }); // Reset errors
      fetchServices();
    } catch (error) {
