...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const fetchAll = async (event?: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    setLoading(true);
    try {
      let payments: PaymentRecord[] = [];
      let loyaltyCustomers: LoyaltyCustomer[] = [];
      ...
      if (isOnline()) {
        ...
        payments = paymentsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })) as PaymentRecord[];
        loyaltyCustomers = loyaltySnap.docs.map(doc => ({ id: doc.id, ...doc.data() })) as LoyaltyCustomer[];
        ...
        // Merge in local records not yet synced
        try {
          const localPayments = await getAllLocal("payments") as PaymentRecord[] || [];
          for (const local of localPayments) {
            const match = payments.find(f =>
            ...
            if (!match) payments.push(local);
          }
          const localLoyalty = await getAllLocal("loyalty_customers") as LoyaltyCustomer[] || [];
          for (const local of localLoyalty) {
            const match = loyaltyCustomers.find(f =>
              f.name === local.name &&
              JSON.stringify(f.cars) === JSON.stringify(local.cars)
            );
            if (!match) loyaltyCustomers.push(local);
          }
          const localEmployees = await getAllLocal("employees") as Employee[] || [];
          for (const local of localEmployees) {
            const match = employees.find(f =>
              f.firstName === local.firstName &&
              f.lastName === local.lastName
            );
            if (!match) employees.push(local);
          }
          const localServices = await getAllLocal("services") as Service[] || [];
          for (const local of localServices) {
            const match = services.find(f =>
              f.name === local.name &&
              JSON.stringify(f.prices) === JSON.stringify(local.prices)
            );
            if (!match) services.push(local);
          }
        } catch {}
      } else {
        // Offline: load from IndexedDB only
        payments = await getAllLocal("payments") as PaymentRecord[] || [];
        loyaltyCustomers = await getAllLocal("loyalty_customers") as LoyaltyCustomer[] || [];
        employees = await getAllLocal("employees") as Employee[] || [];
        services = await getAllLocal("services") as Service[] || [];
      }
      setPayments(payments);
      setLoyaltyCustomers(loyaltyCustomers);
