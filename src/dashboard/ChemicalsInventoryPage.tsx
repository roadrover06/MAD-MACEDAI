...
const ChemicalsInventoryPage: React.FC<{
  ...
  const fetchChemicals = async () => {
    ...
    try {
      ...
      if (isOnline()) {
        ...
        chems = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Chemical[];
        // Merge in local chemicals not yet synced
        try {
          const localChems = await getAllLocal("chemicals") as Chemical[] || [];
          for (const local of localChems) {
            const match = chems.find(c =>
              c.name === local.name &&
              c.unit === local.unit &&
              c.description === local.description
            );
            if (!match) chems.push(local);
          }
        } catch {}
      } else {
        // Offline: load from IndexedDB only
        chems = await getAllLocal("chemicals") as Chemical[] || [];
      }
      setChemicals(chems);
    } catch (error) {
  ...
  const handleAddChemical = async () => {
    ...
    try {
      if (isOnline()) {
        ...
        await setLocal("chemicals", { ...newChemical, id: newId });
        await queueSync("add", "chemicals", { ...newChemical, id: newId });
      }
      ...
      setAddFormErrors({ name: false, stock: false });
      fetchChemicals();
    } catch (error) {
  ...
  const handleEditChemical = async () => {
    ...
    try {
      if (isOnline()) {
        ...
        await setLocal("chemicals", selectedChemical);
        await queueSync("update", "chemicals", selectedChemical);
      }
      setSnackbar({ open: true, message: "Chemical updated successfully!", severity: "success" });
      setEditDialogOpen(false);
      setSelectedChemical(null);
      setEditFormErrors({ name: false, stock: false });
      fetchChemicals();
    } catch (error) {
  ...
  // --- Delete chemical: support offline ---
  ...
  const confirmDeleteChemical = async () => {
    ...
    try {
      if (isOnline()) {
        ...
        await deleteLocal("chemicals", selectedChemical.id);
        await queueSync("delete", "chemicals", { id: selectedChemical.id });
      }
